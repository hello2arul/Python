...
class Register(Resource):
    def post(self):
        ...
        #stores username and password in MongoDB
        users.insert({
            "Username": username,
            "Password": hashed_pw,
            "Own":0,
...
class Add(Resource):
    def post(self):
        ...
        if money <= 0:
            return jsonify(generateReturnDictionary(304, "Invalid amount"))

        cash = cashWithUser(username)
        money -= 5 #Transaction fee
        #Add transaction fee to bank account
        bank_cash = cashWithUser("BANK")
        updateAccount("BANK", bank_cash + 5)
...
class Transfer(Resource):
    def post(self):
        ...
        cash = cashWithUser(username)
        if cash <= 0:
            return jsonify(generateReturnDictionary(303, "You are broke"))

        if money<=0:
            return jsonify(generateReturnDictionary(304, "Invalid Amount"))

        if not UserExist(to):
            return jsonify(generateReturnDictionary(301, "To User is invalid"))
        ...
        updateAccount("BANK", bank_cash + 5)
        updateAccount(to, cash_to + money - 5)
